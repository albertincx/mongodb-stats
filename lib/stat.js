...
MongoStat.prototype.all = function() {
  const obj = {};
  obj.$project = {
    d: {
      $dateToString: {
...
MongoStat.prototype.parseParams = function(query) {
  let filter = false;
  this.query = query;
  const { type = 't', limit, ldate: lastDate } = this.query;
  let { search: s } = this.query;
  let { ldate: lldate, ...search } = s;

  let ldate = lldate;
  if (lastDate) ldate = lastDate;
...
MongoStat.prototype.getArrgegation = function(query) {
  ...
  if (avg || max) {
    ...
    if (max) {
      ...
      if (type !== ALLTIME) obj.$group._id['d2'] = '$d2';
    }
